...
AC_DEFUN([BITCOIN_FIND_BDB53],[
  ...
  if test "$BDB_CFLAGS" = ""; then
    ...
    if test "x$bdbpath" = "xX"; then
      AC_MSG_RESULT([no])
      AC_MSG_ERROR([libdb_cxx headers missing, Bells Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  ...
  if test "$BDB_LIBS" = ""; then
    ...
    if test "x$BDB_LIBS" = "x"; then
        AC_MSG_ERROR([libdb_cxx missing, Bells Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
    fi
  fi
  AC_SUBST(BDB_LIBS)
])

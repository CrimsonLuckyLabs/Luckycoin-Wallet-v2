...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "bells.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("bellsd", "bellsd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: bellsd started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("bellsd", "bellsd")
    args = [ binary, "-datadir="+datadir, "-server", "-keypool=1", "-discover=0", "-rest", "-mocktime="+str(get_mocktime()) ]
    if extra_args is not None: args.extend(extra_args)
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: bellsd started, waiting for RPC to come up")

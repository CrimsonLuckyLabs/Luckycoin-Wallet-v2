...
3. All content of the data directory, except for the `luckcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `luckcoin.conf`       | User-defined configuration settings for `luckcoind` or `luckycoin-qt`; can be specified by `-conf` option
`./`               | `luckcoind.pid`        | Stores the process ID (PID) of `luckcoind` or `luckycoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `luckcoind` or `luckycoin-qt`

...
`./`               | `dogecoin.conf`       | User-defined configuration settings for `dogecoind` or `bells-qt`; can be specified by `-conf` option
`./`               | `dogecoind.pid`        | Stores the process ID (PID) of `dogecoind` or `bells-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `dogecoind` or `bells-qt`

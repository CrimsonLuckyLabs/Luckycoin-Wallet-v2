...
`./`               | `bells.conf`       | User-defined configuration settings for `bellsd` or `luckycoin-qt`; can be specified by `-conf` option
`./`               | `bellsd.pid`        | Stores the process ID (PID) of `bellsd` or `luckycoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bellsd` or `luckycoin-qt`

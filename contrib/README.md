...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Dogecoin network. This means one can have an always-on bellsd instance running, and another local bellsd/bells-qt instance which connects to this node and receives blocks from it.
...
Contains files used to package bellsd/bells-qt
